...
class UsersFile extends Records
{
    ...
    {
        $userToBlock = $this->get($id);
        if ($userToBlock != null) {
            if ($userToBlock->blocked() != 0) {
                $userToBlock->setBlocked(0);
            } else {
                $userToBlock->setBlocked(1);
            }
            parent::update($userToBlock);
        }
    }
    public function updateType($id)
    {
        $userToUpdate = $this->get($id);
        if ($userToUpdate != null) {
            if ($userToUpdate->isAdmin()) {
                $userToUpdate->setType(0);
            } else {
                $userToUpdate->setType(1);
            }
            parent::update($userToUpdate);
        }
    }
